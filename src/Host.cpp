...
Host::Host(int myId){
 ...
 /* Create Threads */
 auto handleSendHelloThread = std::thread(&Host::
   sendHello, this);
...
void Host::publishData(std::string keyword, std::string payload){
 ...
 if(!publisherKeywordData_.keywordExists(keyword)){
  ...
  userInterfaceLock_.lock();
  std::cout << "ERROR:: Keyword: " << keyword << " does not exists." << std::endl;
  userInterfaceLock_.unlock();
  return;
 }
...
void Host::handleDataPackets() {
 ...
 uniqueId = 0;
 struct DataPacketHeader dataPacketHeader;
 bcopy(pending->packet + PACKET_HEADER_LEN, &dataPacketHeader, DATA_HEADER_LEN);
  ...
  switch(userInput){
   ...
   case 'p':
    ...
    switch(publishTypeInput){
     ...
     case 'p':
      userInterfaceLock_.lock();
      std::cout << "Please enter the keyword for which to Publish Data"<< std::endl;
      std::cin >> keywordInput;
     ...
     default:
     std::cout << "Wrong Input entered. Skipping the request." << std::endl;
    }
    break;
